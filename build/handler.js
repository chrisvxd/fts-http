...
const require_handler_function_1 = require("./require-handler-function");
const DEV = process.env.NODE_ENV === 'development';
function createHttpHandler(definition, jsFilePathOrModule, options = {
    cors: {
    ...
    const innerHandler = require_handler_function_1.requireHandlerFunction(definition, jsFilePathOrModule);
    ...
    const handler = (req, res) => {
        const context = new http_context_1.HttpContext(req, res);
        console.log('handle!');
...
async function getParams(context, definition) {
    let params = {};
...
function send(context, code, obj = null) {
    const { res } = context;
    res.statusCode = code;
    ...
    if (obj instanceof stream_1.Stream || is_stream_1.readable(obj)) {
        if (!res.getHeader('Content-Type')) {
            res.setHeader('Content-Type', 'application/octet-stream');
        }
        obj.pipe(res);
...
function sendError(context, error, statusCode) {
